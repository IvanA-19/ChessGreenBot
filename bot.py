...
class Bot(DbHandler, TeleBot):
    ...
    def start(self):
        ...
        def open_menu(message):
            keyboard = self.get_reply_keyboard(['Меню'])
            self.send_action(message.chat.id, bot_chat_actions['text'])
            bot.send_message(message.chat.id, "Чтобы открыть меню - нажмите кнопку", reply_markup=keyboard)
        ...
        def process_inline_keyboard(call):
            if call.data == 'start':
                ...
                bot.edit_message_text("Сколько пешек стоит ферзь?", call.message.chat.id, call.message.id,
                                      reply_markup=keyboard)
                ...
                keyboard = get_inline_keyboard(['Т. Петросян', 'М. Таль', 'С. Карякин'],
                                               [f'var_2_{i}' for i in range(3)])
                bot.edit_message_text("Кому принадлежит знаменитая фраза:"
                ...
                bot.edit_message_text("Сколько официальных чемпионов мира по шахматам известно на сегодняшний день?",
                                      call.message.chat.id, call.message.id, reply_markup=keyboard)
                ...
                keyboard = get_inline_keyboard(['Открытые', 'Закрытые', 'Открытые'],
                ...
                keyboard = get_inline_keyboard(['Роберт Фишер', 'Генри Гроб', 'Вильгельм Стейниц'],
                ...
                keyboard = get_inline_keyboard(['Шах умер', 'Шах потерявший шанс', 'Шах взят в плен'],
                                               [f'var_011_{i}' for i in range(3)])
                bot.edit_message_text("Что означает слово «шахматы» в переводе с персидского языка?",
                                      call.message.chat.id, call.message.id, reply_markup=keyboard)
                ...
                keyboard = get_inline_keyboard(['Королем и 2 конями', 'Королем и ладьей', '2 ладьями'],
                ...
                bot.edit_message_text("Сколько пешек присутствует в начале игры на стандартной шахматной доске?",
                                      call.message.chat.id, call.message.id, reply_markup=keyboard)
